...
- Tools: `discover_target`, `design_ligands`, `design_thermal_switch`, `verify_with_bionemo`, `run_full_pipeline`, `design_promoter_flash`, `screen_and_verify`, `autonomous_design_loop`, `kill_flash_endpoints` — each wraps existing project code rather than reimplementing it (see `mcp_geno_thermal.py` module docstring for the mapping). `autonomous_design_loop` is the one net-NEW capability (not a wrapper): it closes the loop `screen_and_verify` leaves open — iterates design (project Boltz-2/Flash fold = SEARCH oracle) → independent BioNeMo verify (= ACCEPTANCE oracle) → mutate survivors, until N binders are independently corroborated. The two oracles are deliberately different models so the search can't overfit its own scorer; it stops honestly (`fold_oracle_available: false`) rather than evolving mutated variants on invented fitness when no live fold runs. `design_promoter_flash` runs the GA on the RunPod Flash fleet and returns the live 0→N→0 autoscaling metrics; `screen_and_verify` includes them when `use_flash=True` (now the default on `design_ligands`/`run_full_pipeline`/`screen_and_verify`). `kill_flash_endpoints(confirm=True)` runs `flash undeploy --all --force` — destructive, affects every endpoint on the account, never invoke casually.
